...
namespace jlib
{
	...
	FileManager::FileManager(const string& filepath)
	{
		...
		if (!_fin.is_open())
			throw runtime_error(string("FileManager::FileManager(const string& filepath) threw an exception: Could not find file."));
	}

	FileManager::FileManager(const path& filepath)
	{
		_filepath = filepath;
		_fin.open(_filepath);
	...
	{
		_fin.close();
		_fout.open(_filepath, ios_base::app);
